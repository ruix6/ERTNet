...
for train_index, test_index in kf.split(data):
    ...
    #设置检查点, 保存最佳模型
    checkpoint = ModelCheckpoint(filepath=f'model/model_best_within_cl_{i}.h5', 
    ...
    evaluate.plot_ROC(test_labels, pred, labels=['Excited', 'Content', 'Angry', 'Sad'], 
                      ...
                      file=f'picture/roc_within_cl_{i}.jpg',dpi=600)#添加file参数可以将图片保存
    #绘制混淆矩阵
    evaluate.plotHeatMap(np.argmax(test_labels,axis=1), np.argmax(pred,axis=1),
                         ClassSet=['Excited', 'Content', 'Angry', 'Sad'], 
                         file=f'picture/hm_within_cl_{i}.jpg',dpi=600)#添加file参数可以将图片保存
