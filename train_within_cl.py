...
import numpy as np
#可选用的模型
from transformer import CNN_BiLSTM
import keras
...
for train_index, test_index in kf.split(data):
    ...
    #设置检查点, 保存最佳模型
    checkpoint = ModelCheckpoint(filepath=f'model/model_best_within_cl_{i}.h5', monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
    #导入模型
    ...
    #绘制ROC曲线图
    evaluate.plot_ROC(test_labels, pred, labels=['Excited', 'Content', 'Angry', 'Sad'], colorlist=['navy', 'pink', 'cyan', 'darkviolet'], file=f'picture/roc_within_cl_{i}.jpg',dpi=600)#添加file参数可以将图片保存
    #绘制混淆矩阵
    evaluate.plotHeatMap(np.argmax(test_labels,axis=1), np.argmax(pred,axis=1),ClassSet=['Excited', 'Content', 'Angry', 'Sad'], file=f'picture/hm_within_cl_{i}.jpg',dpi=600)#添加file参数可以将图片保存
    cl_logger.info(f"{i} Fold Training Epochs Accuracy: {list(hist.history['val_accuracy'])}")
    i = i + 1
#打印模型参数
model.summary()
#保存
